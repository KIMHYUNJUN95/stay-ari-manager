...
function normalizeCountry(r) {
  ...
  // ISO 2자리 국가코드는 Intl로 한국어 국가명 변환 (locale "ko" = 한국어로 표시)
  if (/^[A-Z]{2}$/.test(raw)) {
    try {
...
function getBookingAmount(r) {
  return Number(r.totalPrice ?? r.price) || 0;
}

/** 일일로그와 동일 기준: 회사·confirmed·OTA(A/B)·다이쿄초 제외·매출금액 > 0 */
function otaConfirmedFilter(rows) {
  return rows
    ...
    .filter((r) => r.building !== "다이쿄초")
    .filter((r) => getBookingAmount(r) > 0);
}
...
async function createOrReplaceSheet(sheets) {
  ...
  const add = await sheets.spreadsheets.batchUpdate({
    spreadsheetId: SPREADSHEET_ID,
    resource: { requests: [{ addSheet: { properties: { title: SHEET_TITLE, tabColorStyle: { rgbColor: { red: 0.95, green: 0.61, blue: 0.07 } } } } }] },
  });
...
async function uploadSheet(buildingData) {
  ...
  const reportTitle = `인원별 점유 현황 리포트 (${START.slice(0, 7)})`;
  const reportMeta = `기간: ${START} ~ ${END} | 기준: bookDate + confirmed + OTA + 다이쿄초 제외 + 매출>0 (일일로그 예약건수와 동일) | 갱신주기: 매일 08:50 JST | 업데이트: ${nowJst} JST`;
  ...
  for (const b of buildingNames) {
    ...
    values.push([`분석 기간: ${START} ~ ${END} (bookDate·confirmed·OTA·다이쿄초 제외·매출>0, 일일로그와 동일)`, "", "", "", "", "", "", "", "", "", "", ""]);
    values.push(["", "", "", "", "", "", "", "", "", "", "", ""]);
  ...
  values.push(["기준: bookDate + confirmed + OTA + 다이쿄초 제외 + 매출>0 (일일로그와 동일)", "", "", "", "", "", "", "", "", "", "", ""]);
...
async function runPaxOccupancyReport(options = {}) {
  ...
  SHEET_TITLE = `인원현황_${yearMonth.replace("-", "_")}`;
  ...
  const buildingNames = Object.keys(agg)
    .filter((k) => k !== "__past3" && agg[k] && typeof agg[k] === "object" && agg[k].rooms)
    .sort((a, z) => a.localeCompare(z, "ko"));
  ...
  for (const b of buildingNames) {
    ...
    grand.four += t.four;
    grand.fivep += t.fivep;
    for (const [c, n] of Object.entries(agg[b].countries)) {
      allCountries[c] = (allCountries[c] || 0) + n;
    }
  }

  const pctVal = (n, d) => (d > 0 ? ((n * 100) / d).toFixed(1) : "0.0");
  const countryTop10 = Object.entries(allCountries)
    .sort((a, z) => z[1] - a[1])
    .slice(0, 10)
    .map(([country, count]) => ({ country, count, pct: pctVal(count, grand.total) }));

  const paxDataForNotion = {
    ...
    grandOne: grand.one,
    grandTwo: grand.two,
    grandThree: grand.three,
    grandFour: grand.four,
    grandFivep: grand.fivep,
    countryTop10,
    past3Months: past3,
    buildings: buildingNames.map((name) => {
      const t = agg[name].total;
      const countries = Object.entries(agg[name].countries)
        .sort((a, z) => z[1] - a[1])
        .slice(0, 10)
        .map(([country, count]) => ({ country, count, pct: pctVal(count, t.total) }));
      const rooms = Object.entries(agg[name].rooms)
        .sort((a, z) => a[0].localeCompare(z[0], "ko"))
        .map(([room, r]) => ({
          room,
          total: r.total,
          one: r.one,
      ...
      return {
        ...
        four: t.four,
        fivep: t.fivep,
        countryTop10: countries,
        rooms,
      };
    }),
  };

  return { sheetTitle: SHEET_TITLE, rowCount: rows.length, start: START, end: END, paxDataForNotion };
}
