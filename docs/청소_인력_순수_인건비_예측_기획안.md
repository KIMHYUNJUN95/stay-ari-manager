...
| 2-person, end at 14:00 | 2 persons x 4 hours | 8 person-hours |
| 2-person, end at 15:00 | 2 persons x 5 hours | 10 person-hours |
...
Clarification: `rowIdx` is the 0-indexed position in the `values[]` array. In the Google Sheets API, row numbers are 1-indexed. So:
...
운영자가 수기로 확정한 운영 인력값을 시스템 권장값과 분리해 보존하기 위한 섹션을 단일 시트 내에 추가한다.

### A. 시트 위치 및 컬럼
- `청소인력예측` 시트 단일 탭 내, `설정 | 건물별 청소 기준` 아래 / `API 입력 예정 데이터` 위에 위치.
- 섹션 제목: `운영 입력값`.
- 컬럼: `월` (YYYY-MM) / `확정 고정인원` / `평균 주근무일수` / `메모`.
- 데이터 행은 노란 배경으로 시각적으로 강조.

### B. 영속성 (Persistence)
- 매일 동기화 스크립트는 운영 입력값 영역의 셀을 **clear / overwrite 하지 않는다**.
- 빌딩 요약 블록의 `deleteDimension` anchor에 운영 입력값 행이 포함되어, 보충 삭제가 운영 입력값 위에서 멈춘다.
- 기존 시트에 `운영 입력값` 섹션이 없으면 동기화 스크립트가 `API 입력 예정 데이터` 앵커 위에 자동 생성한다.
- 예측 대상 월이 섹션에 없으면 `insertDimension`으로 행만 추가하고 기존 입력값은 그대로 둔다.
- 셋업 스크립트는 시트 전체를 다시 생성하므로 관리자만 실행한다.

### C. 인력 산식
```
system_fixed_staff       = baselineOperational
applied_fixed_staff      = manual.confirmed_fixed_staff ?? system_fixed_staff
avg_part_time_days       = manual.avg_part_time_days_per_week ?? AVG_WORK_DAYS_PER_WEEK   // 기본 3
part_time_availability   = avg_part_time_days / 7

support_pool        = CEILING( MAX(0, peakOperational - applied_fixed_staff) / part_time_availability )
total_required_pool = applied_fixed_staff + support_pool
fixed_shortage      = MAX(0, system_fixed_staff - applied_fixed_staff)
all_part_time_equivalent = CEILING( peakOperational / part_time_availability )   // 참고값
```

### D. 라벨 매핑
- 월별 캘린더 KPI 1행: `월 예상 총 청소 / 최소 고정인원 / 적용 고정인원 / 고정 부족`.
- 월별 캘린더 KPI 2행: `확정/예상 CO / 보충풀 필요 / 총 확보 필요 / 예상인건비(Base)`.
- 월별 요약: `월 / 총 확보 필요 / 시스템 고정권장 / 적용 고정인원 / 고정 부족 / 보충풀 필요 / 전체 주3일 환산 / 피크일`.

### E. `총 확보 필요` vs `전체 주3일 환산`
- `총 확보 필요`는 실제로 확보해야 할 사람 수 (`applied_fixed_staff + support_pool`).
- `전체 주3일 환산`은 모든 인력을 알바로 가정했을 때의 이론값 (`CEILING(peak / (avg_days/7))`). 참고용.
- 두 값은 `applied_fixed_staff > 0`일 때 항상 다르며, `총 확보 필요 ≤ 전체 주3일 환산`.

### F. 수동 검증 시나리오
- 2026-05 행에 `확정 고정인원 = 6`, `평균 주근무일수 = 3` 입력.
- sync 실행 후, 2026-05 KPI의 `보충풀 필요 = CEILING((peak - 6) / (3/7))`, `총 확보 필요 = 6 + 보충풀 필요`임을 확인.
- 다시 sync 실행 시 입력값이 그대로 유지되어야 함.
